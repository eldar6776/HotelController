...

{
    ...
    // Default na NOVI protokol da se izbjegne neinicijalizovano stanje

    m_settings.rx_tx_delay_ms = RX2TX_DEL_MS;

    m_settings.response_timeout_ms = RS485_RESP_TOUT_MS;

    m_settings.packet_payload_size = UPDATE_DATA_CHUNK_SIZE;

    m_settings.use_single_byte_response = false;
...

{
    ...
    Serial.println(F("[Rs485Service] Inicijalizacija..."));
        ...
        // Učitaj protokol iz konfiguracije

    

        SetProtocol(static_cast<ProtocolVersion>(g_appConfig.protocol_version));
...
void Rs485Service::SetProtocol(ProtocolVersion version)

{

    // Ovdje mapiramo protokole na STARI ili NOVI tip

    // STARI PROJEKT (SAX, VUCKO?, itd.) -> Provjeriti tačno mapiranje

    // Za sada pretpostavljamo: SAX = STARI, BASKUCA = NOVI (Default)

    

    // Reset na NOVI (Default)

    m_settings.rx_tx_delay_ms = 3;      // 3ms

    m_settings.response_timeout_ms = 45;// 45ms

    m_settings.packet_payload_size = 128;

    m_settings.use_single_byte_response = false;



    if (version == ProtocolVersion::SAX) // Dodati ostale stare protokole po potrebi

    {

        Serial.println(F("[Rs485Service] Protokol: STARI (SAX Type)"));

        m_settings.rx_tx_delay_ms = 10;     // 10ms

        m_settings.response_timeout_ms = 78;// 78ms

        m_settings.packet_payload_size = 64;

        m_settings.use_single_byte_response = true;
...

    {
        ...
        Serial.println(F("[Rs485Service] Protokol: NOVI (Default/BASKUCA Type)"));
...

{
    ...
    // Za Single Byte Response protokol, ako je dužina 1, a bajt je ACK ili NAK, to je validno.

    if (m_settings.use_single_byte_response && length == 1) {

        if (buffer[0] == ACK || buffer[0] == NAK) return true;

        // Ako nije ACK/NAK, nastavlja dalje na standardnu validaciju (možda je STX paket od klijenta)
...

{
    ...
    // Ako nije specificiran timeout (0), koristi sistemski

    if (timeout_ms == 0) {

        timeout_ms = m_settings.response_timeout_ms;
...

    {
...

        {
            ...
            // KORAK 0: Specijalna obrada za Single Byte Response (Stari protokol)

            if (m_settings.use_single_byte_response && rx_count == 1)

            {
...

                {
                    ...
                    Serial.printf("[Rs485] -> Single Byte Response: 0x%02X\n", buffer[0]);

                    return 1; // Odmah vrati 1 bajt

                }

                // Ako nije ACK/NAK, nastavi čitati (možda je STX paket sa podacima)
...

{
    ...
    // Dinamička pauza između prijema i slanja (RX to TX delay)

    // Samo ako smo nešto primili neposredno prije (simulirano) - ili uvijek za sigurnost

    delay(m_settings.rx_tx_delay_ms);
